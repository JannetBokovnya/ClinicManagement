...
namespace ClinicManagement.Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        private  IClinicService<Specialization> _clinicSpecService;
        private readonly IApplicationUserServise _applicationUser;

        [InjectionConstructor]
        public AccountController(IClinicService<Specialization> clinicSpecService, IApplicationUserServise applicationUser)
        {
        ...
        //[InjectionConstructor]
        //public AccountController(IApplicationUserServise applicationUser)
        //{
        //    _applicationUser = applicationUser;
        //}
        ...
        public ActionResult RegisterDoctor()
        {
            ...
            //using (var clinicContext = new ClinicContext())
            //{
            //    result = clinicContext.Set<Specialization>().ToList();
            //}
            var spec = _clinicSpecService.GetAll().ToList();
            var viewModel = new DoctorFormViewModel()
            {
                Specializations = spec
                //Specializations = _clinicSpecService.GetAll()
        ...
        public async Task<ActionResult> RegisterDoctor(DoctorFormViewModel viewModel)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    //прежде надо заполнить таблицу ролей AspNetRoles(1- Administrator, 2-Doctor)
                    ...
                    UserManager.AddClaim(user.Id, new Claim(ClaimTypes.GivenName, (doctor.FirstName)));
                    //Mapper.Map<DoctorFormViewModel, Doctor>(model, doctor);
                    
                    using (var clinicContext = new ClinicContext())
                    {
                        clinicContext.Set<Doctor>().Add(doctor);
        ...
        //пока временно для проверки логина авторизации,
        [AllowAnonymous]
        public ActionResult Index()
        {
            var usersWithRoles = _applicationUser.GetUsers();
        ...
        public ActionResult Edit(string id)
        {
            ...
            var user = _applicationUser.GetUser(id);
            if (user == null)
            {
                return HttpNotFound();
            }
        ...
        public ActionResult Edit(UserViewModel editUser)
        {
            if (ModelState.IsValid)
            {
                var user = _applicationUser.GetUser(editUser.Id);
                if (user == null)
                {
                    return HttpNotFound();
                }
                _applicationUser.Update(editUser);
